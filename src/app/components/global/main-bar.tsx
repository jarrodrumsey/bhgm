...
import ProgressInfo, { CountDownItemProps } from '../progress-info'
...
const MainBar = (props:{progressValue?:number, max?:number, countdowns:CountDownItemProps[]}) => {
  return (
    ...
    flex p-4 flex-col gap-2 ">
        ...
        <ProgressBar value={props.progressValue ?? 1} max={props.max ?? 1} color="bg-gradient-to-r from-red-500 to-yellow-500" background="bg-slate-400">
            <ProgressInfo countdowns={props.countdowns} />
        </ProgressBar>
    </div>
  )
}
